...
report ambiguity without revealing managed paths. The root is normalized with
the managed absolute instance-root scalar rules and must not contain colon,
...
among strict `ROOT/NAME.identity` indexes, then reads the exact
...
report ambiguity without exposing managed paths. Both forms require the profile
...
`ROOT/profiles` must be a real directory and manifests must be regular files,
never symlinks. New entries request modes `0700` and `0600`, but
filesystem-reported ownership and permission bits are not admission criteria. A
final manifest is atomically created without replacing an existing record;
concurrent byte-identical publishers reuse it, while any malformed, unstable,
or conflicting record fails closed. The internal
